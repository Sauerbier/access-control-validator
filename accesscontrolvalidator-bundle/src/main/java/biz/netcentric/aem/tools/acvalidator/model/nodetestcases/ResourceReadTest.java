 ...
 */
public class ResourceReadTest extends ResourceTestCase implements SimulatableTest, Testable{
	
	private final Logger LOG = LoggerFactory.getLogger(ResourceReadTest.class);
	 ...
	 * @param simulate read
	 */
	public ResourceReadTest(String path, boolean isAllow, boolean simulate) {
	...
	public TestResult isOk(ResourceResolver serviceResourcerResolver, ResourceResolver testUserResolver, Authorizable authorizable) throws RepositoryException, LoginException {
		...
		return new TestResult(authorizable.getID(), "Resource Read Test", " path: " + path + ", isSimulate:" + this.simulate + " isSimulateSuccess: " + this.isSimulateSuccess(), isOk , this.errorMessage);
	}
